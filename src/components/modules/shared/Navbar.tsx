...
const NavLinks = () => (
  <>
    ...
    <Link href="/medicine" className="hover:text-primary-foreground/80 transition-colors">Browse Medicine</Link>
    <Link href="/about" className="hover:text-primary-foreground/80 transition-colors">About Us</Link>
    <Link href="/contact" className="hover:text-primary-foreground/80 transition-colors">Contact Us</Link>
  </>
);
...
export default function Navbar() {
  ...
  // If not mounted, render a minimal version or nothing to avoid hydration mismatch
  if (!isMounted) return <header className="w-full bg-primary py-4 px-6 shadow-md" />;

  return (
    <header className="w-full bg-primary py-4 px-6 sticky top-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center gap-10">
          <Link href="/" className="flex items-center gap-2 font-bold text-primary-foreground text-xl tracking-tight">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <Button variant="ghost" size="icon" className="text-primary-foreground hover:bg-white/20 rounded-full hidden sm:flex">
          ...
          <div className="hidden md:flex items-center gap-2">
            {!isPending ? (
              session ? (
                <Button onClick={handleLogout} className="border-primary-foreground border text-primary-foreground hover:text-secondary-foreground font-medium">
                  Logout
                </Button>
              ) : (
                <>
                  <Button variant="ghost" className="border-primary-foreground border text-primary-foreground hover:text-secondary-foreground font-medium">
              ...
              // Simple skeleton placeholder while session loads to prevent button jump
              ...
              <SheetContent side="right" className="bg-primary border-2 border-red-600 px-4 pt-4 text-primary-foreground">
                <nav className="flex flex-col gap-4 text-lg font-semibold">
                  ...
                  <div className="flex flex-col gap-4">
                     {!isPending && (
                        session ? (
                            <Button onClick={handleLogout} className="bg-white text-primary font-bold">Logout</Button>
                        ) : (
                            <>
                                <Button variant="outline" className="text-primary border-primary-foreground hover:bg-white/20">
                                    <Link href="/login">Login</Link>
                                </Button>
